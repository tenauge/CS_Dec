...
def DecodeMetadata(decrypted, oOutput):
    ...
    oOutput.Line('Raw key（Hex）:  %s' % binascii.b2a_hex(rawkey).decode())
    sha256hex = hashlib.sha256(rawkey).hexdigest()
    aeskey = sha256hex[:32]
    hmackey = sha256hex[32:]
    oOutput.Line(Fore.RED + Style.BRIGHT + ' AES_Key（Hex）:  %s' % aeskey)
    oOutput.Line(Fore.RED + Style.BRIGHT + ' HMAC_Key（Hex）: %s' % hmackey)
    
    # ------- 保存十六进制密钥到 JSON -------
    data = {
        "AES_Key_Hex": aeskey,
        "HMAC_Key_Hex": hmackey
    }
    ...
    print(Fore.GREEN + Style.BRIGHT + "已保存十六进制形式的 AES 和 HMAC 密钥到 ./keys.json ✔")
    ...
    if not re.match(rb'[0-9]+\t[0-9]+\t[0-9]', peek):
        bid, pid, port, flags = oStruct.Unpack('>IIHB')
        oOutput.Line('\nBeacon id: %d' % bid)
        oOutput.Line('Beacon pid: %d' % pid)
        ...
        if not re.match(rb'[0-9]+\.[0-9]+\t[0-9]+', peek):
            ...
            oOutput.Line('\n受控端 IPv4: %s' % '.'.join([str(byte) for byte in ipv4[::-1]]))

    remainder = oStruct.GetBytes()
    fields = remainder.split(b'\t')

    # 预期的标签
    labels = ["受控端主机名: ", "受控端用户名: ", "后门程序名称: "]

    for i, field in enumerate(fields):
        text = field.decode(errors="ignore")

        if i < len(labels):
            # 输出带标签的字段
            oOutput.Line(f"{labels[i]}{text}")
        else:
            # 超出数量的字段仍然保留 Field 输出
            oOutput.Line(f"Field: {text}")
...
def Main():
    init(autoreset=True)  # 初始化，自动重置颜色
    moredesc = '''
Source code put in the public domain by Didier Stevens, no Copyright
Use at your own risk
https://DidierStevens.com'''
